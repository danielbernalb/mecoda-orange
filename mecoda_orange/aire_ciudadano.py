...
def get_data(url, selected_cols):
    ...
    df = df.drop(columns=[col for col in df.columns if "metric." in col]).reset_index(
        drop=True
    )
...
class AireCiudadanoWidget(OWBaseWidget):
    ...
    class Outputs:
        registers = Output("Registers", Orange.data.Table, auto_summary=False)
    ...
    def __init__(self):
        ...
        self.parametersBox = gui.widgetBox(
            self.controlArea, "Range options", orientation=1, disabled=True
        )

        self.startsBox = gui.widgetBox(self.parametersBox, "Starts", orientation=1)
        self.starts_day_line = gui.lineEdit(
            self.startsBox, self, "starts_day", label="<b>Day</b>", controlWidth=80
        )
        self.starts_hour_line = gui.lineEdit(
            self.startsBox, self, "starts_hour", label="<b>Hour</b>", controlWidth=40
        )

        self.endsBox = gui.widgetBox(self.parametersBox, "Ends", orientation=1)
        self.starts_day_line = gui.lineEdit(
            self.endsBox, self, "ends_day", label="<b>Day</b>", controlWidth=80
        )
        self.starts_hour_line = gui.lineEdit(
            self.endsBox, self, "ends_hour", label="<b>Hour</b>", controlWidth=40
        )

        self.stepBox = gui.widgetBox(self.parametersBox, "Step", orientation=2)
        self.step_number_line = gui.lineEdit(
            self.stepBox, self, "step_number", label="", controlWidth=80
        )
        self.step_options_line = gui.comboBox(
            ...
            label="",
            labelWidth=None,
            items=("seconds", "minutes", "hours", "days", "weeks", "years"),
    ...
    def commit(self):
        ...
        try:
            ...
            if self.option == 0:
                url = (
                    f"http://sensor.aireciudadano.com:30000/api/v1/query?query={query}"
                )
            ...
            self.infoa.setText(f"{len(obs):,} registers gathered")
